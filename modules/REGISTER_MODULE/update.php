...
if (preg_match("/^update$/i", $message)) {
	...
	if ($db->numrows() == 0) {
		...
		$sql = "SELECT * FROM warbot.admin_taratime WHERE `name` = '$sender' AND `adminlevel` = 3;";
		$db->query($sql); 
		if ($db->numrows()!=0){
			$sql = "SELECT id, role_id FROM omnihqdb.jos_agora_user_group WHERE user_id = $ag_id AND group_id = $group_id_mod;";
			$db->query($sql);
	...
	$chatBot->send("Updated " . count($users) . " orgs, ranks and {$upd} other entries.", $sendto);
} else {
	$syntax_error = true;
}
