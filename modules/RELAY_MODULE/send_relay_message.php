...
if (($chatBot->settings["relaybot"] != "Off") && ($chatBot->data["leader"]==$sender || isset($chatBot->admins[$sender]["level"])) && ($chatBot->settings["bot_relay_commands"] == 1 || $args[2][0] != $chatBot->settings["symbol"])) {
	$relayMessage = '';
...
*/
		if ($type == "guild") {
			...
			$msg = "grc [<myname>] {$sender}: <yellow>{$relayMessage}<end>";
		}
