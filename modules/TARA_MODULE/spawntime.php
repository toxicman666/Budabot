...
if (preg_match("/^spawntime$/i", $message)) {
	$spawntime = Tara::spawntime();
	
	if ($spawntime->state == 0) {
		$left = Util::unixtime_to_readable($spawntime->time-time());
		$state = "";
	} else {
		$left = Util::unixtime_to_readable($spawntime->manual-time());
		$state = " <red>[manual]<end>";
	}
	
	if ($left>0){
		$msg = "<yellow>Expected in " . $left . "<end> (by " . $spawntime->set_by . "){$state}";
		if ($spawntime->skip>0){
			$msg .= " <highlight>skipped <end><white>" . $spawntime->skip . "<end><highlight> times.<end>";
		}
	} else $msg = "<yellow>Tarasque has spawned.<end>";
	$chatBot->send($msg,$sendto);
} else if (preg_match("/^spawntime ([a-z]+)$/i", $message, $arr)){
	$spawntime = Tara::spawntime();
	...
	if ($timezone[$zone]["name"]) {
		if ($time-time()>0){
			$msg = "<yellow>Expected at " . $timezone[$zone]["time"] . "<end> <highlight>{$zone}, {$timezone[$zone]["name"]}<end> (by " . $spawntime->set_by . "){$state}";
			if ($spawntime->skip>0){
				$msg .= " <highlight>skipped <end><white>" . $spawntime->skip . "<end><highlight> times.<end>";
			}
		} else $msg = "<yellow>Tarasque has spawned.<end>";
