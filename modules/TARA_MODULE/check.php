...
if (preg_match("/^check$/i", $message) || preg_match("/^check all$/i", $message)) {
	...
	$sql = "SELECT o.name, p.profession FROM online_<myname> o LEFT JOIN {$playersdb} p ON o.name = p.name WHERE added_by = '<myname>' AND channel_type = 'priv'";
	$db->query($sql);
	$data = $db->fObject('all');
	forEach ($data as $row) {
		$prof[$row->profession] .= " \\n /assist {$row->name}";
	...
	forEach ($prof as $key => $value) {
		$list .= "<a href='chatcmd:///text Assist {$key}: {$value}'>Check $key</a>\n";
	}
	...
	$playersdb = Player::get_players_db();
	$db->query("SELECT o.name, p.guild FROM online_<myname> o LEFT JOIN {$playersdb} p ON o.name = p.name WHERE added_by = '<myname>' AND channel_type = 'priv' ORDER BY `guild` DESC");
	$data = $db->fObject('all');
	forEach ($data as $row) {
		if ($row->guild == "") {
			$org["Non orged"] .= " \\n /assist $row->name";
