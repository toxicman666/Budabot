...
if (!preg_match("/^.?afk(.*)$/i", $message)) {
	$db->query("SELECT afk FROM online_<myname> WHERE `name` = '{$sender}' AND added_by = '<myname>' AND channel_type = '$type'");
	$row = $db->fObject();

	if ($row != null && $row->afk != '') {
		$db->exec("UPDATE online_<myname> SET `afk` = '' WHERE `name` = '{$sender}' AND added_by = '<myname>' AND channel_type = '$type'");
		...
		if ($uid) {
			$db->query("SELECT afk FROM online_<myname> WHERE `name` = '{$name}' AND added_by = '<myname>'");
